    ...
    <div class="options-wrapper">
        <aside class="tabla-filtros">
            ...
            <article class="tabla-buscador">
                ...
                <input type="text" id="customSearch" placeholder="Orden, cliente, pasarela o gateway ID" autocomplete="off" />
        ...
        <div class="form-row-fit">
            <div class="meta-group">
                ...
                <div class="meta-row-fit">
                    <div><strong>Pagos:</strong> {{ $payments->count() }}</div>
                    <div><strong>Bruto:</strong> S/. {{ number_format($totalBruto, 2) }}</div>
                    <div><strong>Comisiones:</strong> S/. {{ number_format($totalComisiones, 2) }}</div>
                    <div><strong>% comisión global:</strong> {{ number_format($ratioComision, 2) }}%</div>
                    <div><strong>Neto:</strong> S/. {{ number_format($totalNeto, 2) }}</div>
                </div>
            </div>
        </div>
                    ...
                    <table id="tabla" class="tabla-general display">
                        ...
                        <tbody>
                            @foreach ($payments as $payment)
                                ...
                                @endphp
                                <tr data-id="{{ $payment->id }}" data-name="{{ strtoupper((string) $payment->provider) }}">
                                    ...
                                    <td class="column-name-td" data-provider="{{ mb_strtolower((string) $payment->provider) }}">{{ strtoupper((string) $payment->provider) }}</td>
                                    ...
                                    <td class="column-status-td" data-status="{{ mb_strtolower((string) $payment->status) }}">
                                        @switch($payment->status)
                                            @case('pending')
                                                <span class="badge badge-warning"><i class="ri-time-line"></i> Pendiente</span>
                                            @break

                                            @case('paid')
                                                <span class="badge badge-success"><i class="ri-checkbox-circle-line"></i> Pagado</span>
                                            @break

                                            @case('failed')
                                                <span class="badge badge-danger"><i class="ri-error-warning-line"></i> Fallido</span>
                                            @break

                                            @case('refunded')
                                                <span class="badge badge-info"><i class="ri-refund-2-line"></i> Reembolsado</span>
                                            @break

                                            @default
                                                <span class="badge badge-secondary">{{ ucfirst((string) $payment->status) }}</span>
                                        @endswitch
                                    </td>
                                    <td>{{ $payment->transaction_id ?: '—' }}</td>
                                    <td class="column-date-td" data-paid-date="{{ $paidDate ? $paidDate->format('Y-m-d') : '' }}">
                                        {{ $paidDate ? $paidDate->format('d/m/Y H:i') : 'Sin fecha' }}
                                    </td>
                                    <td class="column-actions-td">
                                        <a href="{{ route('admin.payments.show', $payment) }}" class="boton-table boton-editar" title="Ver detalle">
