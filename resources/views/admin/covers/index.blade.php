...
<x-admin-layout :showMobileFab="true">
    ...
    <div class="covers-container">
        ...
        <div class="covers-gallery" id="coversGallery">
            ...
            <div class="cover-card" data-id="{{ $cover->id }}" data-title="{{ $cover->title }}" data-position="{{ $cover->position }}" data-status="{{ $cover->status }}">
                <!-- Checkbox de selección -->
                @can('portadas.delete')
                <div class="card-checkbox">
                    <input type="checkbox" class="check-row" id="check-row-{{ $cover->id }}" value="{{ $cover->id }}">
                </div>
                @endcan

                <!-- Imagen principal -->
                <div class="card-image-wrapper">
                    <img src="{{ asset('storage/' . $cover->image_path) }}" alt="{{ $cover->title }}" class="card-image" data-image-path="{{ $cover->image_path }}">
                    <div class="card-overlay">
                        <div class="overlay-actions">
                            @can('portadas.edit')
                            <a href="{{ route('admin.covers.edit', $cover->slug) }}" class="overlay-btn edit-btn" title="Editar">
                                <i class="ri-pencil-line"></i>
                            </a>
                            @endcan
                            @can('portadas.delete')
                            <button type="button" class="overlay-btn delete-btn" title="Eliminar" data-url="{{ route('admin.covers.destroy', $cover->slug) }}" data-id="{{ $cover->id }}" data-name="{{ $cover->title }}">
                                <i class="ri-delete-bin-line"></i>
                            </button>
                            @endcan
                        </div>
                ...
                <div class="card-content">
                    ...
                    <div class="card-footer">
                        ...
                        <label class="toggle-status">
                            <input type="checkbox" class="switch-status" data-id="{{ $cover->id }}" data-key="{{ $cover->slug }}" {{ $cover->status ? 'checked' : '' }}>
                            <span class="toggle-slider"></span>
                            <span class="toggle-text">{{ $cover->status ? 'Activo' : 'Inactivo' }}</span>
                        </label>
                        @else
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            class CoversGalleryManager {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            showToast({
                                ...
                                message: data.message || 'El estado se actualizó correctamente.',
                                duration: 3000
                            });
                        }
                    })
                ...
                sortCards(cards, sortType) {
                    ...
                    switch(sortType) {
                        case 'position-asc':
                            return sorted.sort((a, b) => parseInt(a.dataset.position) - parseInt(b.dataset.position));
                        case 'position-desc':
                            return sorted.sort((a, b) => parseInt(b.dataset.position) - parseInt(a.dataset.position));
                        case 'title-asc':
