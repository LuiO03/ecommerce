...
async function resolveRecaptchaToken(form) {
    ...
    return window.grecaptcha.execute(siteKey, { action: 'claims_submit' });
}

export function initClaimsForm() {
    ...
    const originalText = submitButton?.dataset.defaultText || 'Enviar registro';
    const statusContainer = document.getElementById('claims-form-status');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (payload.success) {
                setAlert(statusContainer, 'success', payload.message || 'Tu registro fue enviado correctamente.');
                form.reset();
            ...
            const errors = payload.errors || { form: 'Ocurrió un error al procesar tu solicitud.' };
