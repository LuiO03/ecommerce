document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("touchstart", (e) => {
        ...
        if (
            ...
            userSidebar.classList.contains("translate-x-full")
        ) {
    ...
    document.addEventListener("touchmove", (e) => {
        ...
        if (activeSidebar === "left") {
            if (startX < 40 && deltaX > 0) {
                const translate = Math.min(
                    0,
                    -leftSidebar.offsetWidth + deltaX
                );
                leftSidebar.style.transform = `translateX(${translate}px)`;
                showOverlay(
                    20,
                    Math.min(deltaX / leftSidebar.offsetWidth, 0.5)
                );
            } else if (
                deltaX < 0 &&
                !leftSidebar.classList.contains("-translate-x-full")
        ...
        if (activeSidebar === "right") {
            if (startX > window.innerWidth - 40 && deltaX < 0) {
                ...
                showOverlay(
                    60,
                    Math.min(Math.abs(deltaX) / userSidebar.offsetWidth, 0.5)
                );
            } else if (
                deltaX > 0 &&
                !userSidebar.classList.contains("translate-x-full")
